...
export class ObjectPool<T extends ObjectBase> extends ObjectPoolBase implements IObjectPool<T> {
    ...
    public unSpawnTarget(target: any){
        ...
        GlobalApp.LogMgr.error('unspawn target no find target');
    }
    ...
    public releaseByCount(count:number, f:Function){
        ...
        let expireTimie = Date.now() - this._expireTime;
        let releaseArray:Array<T> = f.apply(this, [canRelease, count, expireTimie]);//apply对应数组，call对应obj
        ...
        for(let i = 0;i<releaseArray.length;++i){
            ...
            for(let j = this._pool.length -1;j>=0;--j){
                let rObj = this._pool[j];
                if(rObj.peek().target === obj.target){
                    isFind = true;
                    rObj.release();
